  ...
  outputs =
    ...
    let
      ...
      linkerVar =
        pkgs:
        "CARGO_TARGET_"
        + nixpkgs.lib.toUpper (builtins.replaceStrings [ "-" ] [ "_" ] pkgs.stdenv.hostPlatform.config)
        + "_LINKER";
    in
    {
      # How masys is meant to be installed. crates.io carries the names
      # and nothing else: masys is an application, and `cargo install` is
      # a convenience for Rust developers rather than the way an operator
      # gets a tool. Its one distro-specific buffer is NixOS's, so this is
      # where its users already are.
      packages = forEach (pkgs: rec {
        masys = pkgs.rustPlatform.buildRustPackage {
          pname = "masys";
          version = "0.1.0";
          src = ./.;
          ...
          # Without this masys still runs, and reads the journal by
          # spawning `journalctl -o json` - which works and cannot
          # follow. `sd_journal.rs` is explicit that soname-only `dlopen`
          # fails on NixOS, which keeps no global library path and no
          # `ldconfig` cache, so a package that skipped this would ship
          # the degraded path to precisely the users this output is for.
          #
          # `--set-default`, not `--set`: the variable is documented as a
          # universal escape hatch, and an operator pointing masys at
          # another libsystemd should keep winning over the package.
          postInstall = ''
            wrapProgram $out/bin/masys \
              --set-default MASYS_LIBSYSTEMD "${pkgs.systemd}/lib/libsystemd.so.0"
          '';

          meta = {
            description = "A magit-style TUI for Linux system runtime ops";
            homepage = "https://github.com/sarabande/masys";
