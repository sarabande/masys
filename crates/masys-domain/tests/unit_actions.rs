...
fn start_passes_the_unit_name_through() {
    ...
    assert_eq!(
        service.only_call(),
        Call::Start("restic-backup.service".to_string())
    );
}
...
fn stop_passes_the_unit_name_through() {
    ...
    assert_eq!(
        service.only_call(),
        Call::Stop("otel-collector.service".to_string())
    );
}
...
fn restart_passes_the_unit_name_through() {
    ...
    assert_eq!(
        service.only_call(),
        Call::Restart("restic-backup.service".to_string())
    );
}
...
fn reload_passes_the_unit_name_through() {
    ...
    assert_eq!(
        service.only_call(),
        Call::Reload("nginx.service".to_string())
    );
}
