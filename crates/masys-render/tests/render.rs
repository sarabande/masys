...
use masys_view::{
    ActionGroup, ActionRow, Header, KeyBinding, KeyGroup, ModalView, Node, Overview, SectionKind,
    StatusLine, SwitchRow, View,
};
...
fn draw(hostname: &str, rows: &[Node], status: StatusLine<'_>) -> Vec<String> {
    buffer_lines(&drawn(&Theme::default(), hostname, rows, status))
}

/// One frame at the fixed test size, as the terminal it was drawn on -
/// which is what a test about *colour* needs, since `buffer_lines`
/// above keeps only the symbols.
fn drawn(
    theme: &Theme,
    hostname: &str,
    rows: &[Node],
...
fn the_header_shows_hostname_left_and_timestamp_right() {
    ...
    assert_eq!(
        ...
        WIDTH as usize - 1,
        "the timestamp is flush right"
    );
    assert_eq!(
        lines[1], "",
        "a blank line separates the header from the first row"
    );
}
...
/// A process group heads its section and draws through the same dispatch
/// every other row does.
///
/// **The function was covered and the path to it was not.**
/// `tests/view.rs` calls `proc_group_line` directly, so replacing this
/// row's arm in `render`'s dispatch with a blank line failed nothing,
/// anywhere in the workspace - the leaf-cgroup rule had a test and the
/// wiring that reaches it had none. `producers.rs` makes the same
/// argument about `SectionKind`: coverage of a shape is not coverage of
/// a path to it.
///
/// Placement, like everything else here: what the line *says* is
/// `tests/view.rs`'s business, and this asserts only that saying it
/// reaches the terminal.
#[test]
fn a_process_group_row_reaches_the_frame() {
    let rows = vec![
        Node::ProcGroup {
            name: "/system.slice/nginx.service".to_string(),
            depth: 0,
            expanded: true,
            cpu_percent: 12.5,
    ...
    assert!(
        lines.iter().any(|l| l.contains("nginx.service")),
        "the group names the cgroup it heads: {lines:#?}"
    );
    assert!(
        lines.iter().any(|l| l.contains('v')),
        "an expanded group shows its fold marker: {lines:#?}"
    );
}
...
fn a_failed_unit_row_renders_its_glyph_and_text() {
    let mut rows = vec![
        ...
        Node::finding(Finding::new(FindingKind::FailedUnit {
    ...
    let lines = draw("devbox", &rows, StatusLine::Hints);
    assert!(
        lines.iter().any(|l| l.contains("Failed units (1)")),
        "{lines:#?}"
    );
    assert!(
        lines.iter().any(|l| l.contains("x restic-backup.service")),
        "{lines:#?}"
    );
}

/// The severity a finding is drawn in is the one it was handed, not one
/// the renderer knows.
///
/// The seam `[theme]` in the config file hangs off: every function here
/// takes a `&Theme`, so a palette read from a host's file reaches the
/// cells by the same path the default does. Asserted on the drawn
/// terminal rather than on a `Line`, because the question is what the
/// operator's screen ends up saying - a colour resolved correctly and
/// then dropped on the way to a cell would pass a test on the line and
/// still leave the host with no signal.
///
/// The glyph is checked beside the colour on purpose. `x` carries the
/// shape and the colour carries the judgement, and a test that pinned
/// only one of them would let the other be recoloured to match the row
/// beneath it.
#[test]
fn a_finding_is_drawn_in_the_colour_the_theme_names() {
    let theme = Theme {
        severity_dead: Color::Indexed(208),
        ..Theme::default()
    };
    let mut rows = vec![
        header("Failed units", SectionKind::FailedUnits, Some(1)),
    ...
    let terminal = drawn(&theme, "devbox", &rows, StatusLine::Hints);
    let (column, row) = glyph_of(&terminal, "x restic-backup.service");
    let cell = &terminal.backend().buffer()[(column, row)];
    assert_eq!(
        cell.fg,
        Color::Indexed(208),
        "the failed-unit glyph is drawn in the theme's severity_dead, not the default \
         {:?}",
        Theme::default().severity_dead
    );
}

/// The cell the given row text starts at, as `(column, row)`.
///
/// By the whole row rather than by its first character, which was this
/// helper's first shape and was wrong in a way that passed: the frame's
/// hostname is `devbox`, so a search for the first `x` on the screen
/// found the header and reported an unstyled cell. A glyph is only
/// identifiable together with the row it belongs to.
fn glyph_of(terminal: &Terminal<TestBackend>, row_text: &str) -> (u16, u16) {
    let lines = buffer_lines(terminal);
    let (row, line) = lines
        .iter()
        .enumerate()
        .find(|(_, line)| line.contains(row_text))
        .unwrap_or_else(|| panic!("{row_text:?} is on no line of {lines:#?}"));
    let column = column_of(line, row_text);
...
fn the_footer_shows_a_separator_and_hints() {
    ...
    assert!(
        separator.trim_start().chars().all(|c| c == '-'),
        "{separator:?}"
    );
    assert!(
        separator.starts_with(" -"),
        "the separator is inset too: {separator:?}"
    );
    assert!(
        lines[lines.len() - 1].contains("[t] procs"),
        "{:?}",
        lines[lines.len() - 1]
    );
}
...
fn an_error_status_line_replaces_the_hints() {
    let lines = draw(
        "devbox",
        &system_section(),
        StatusLine::Error("polkit: access denied"),
    );
    assert!(
        lines[lines.len() - 1].contains("polkit: access denied"),
        "{:?}",
        lines[lines.len() - 1]
    );
    assert!(
        !lines.iter().any(|l| l.contains("[r]estart")),
        "an error takes the footer over entirely: {lines:#?}"
    );
}
...
fn a_long_error_cannot_starve_the_header_and_list() {
    ...
    assert!(
        lines[0].starts_with(" masys - devbox"),
        "the header survives a 40-line error: {lines:#?}"
    );
    assert_eq!(
        lines[2], " System",
        "at least one list row survives too: {lines:#?}"
    );
    assert!(
        lines.iter().any(|l| l.contains("line 0")),
        "the error is still shown, truncated: {lines:#?}"
    );
}
...
fn an_empty_row_list_still_draws_the_frame() {
    ...
    assert!(
        lines[2..lines.len() - 2].iter().all(|l| l.is_empty()),
        "no rows means a blank list: {lines:#?}"
    );
}
...
fn findings_in_one_section_share_a_padded_label_column() {
    let mut rows = vec![
        ...
        Node::finding(Finding::new(FindingKind::Pressure {
    ...
    let columns: Vec<usize> = lines
        .iter()
        .filter(|l| l.contains("some avg60"))
        .map(|l| column_of(l, "some avg60"))
        .collect();
    ...
    assert_eq!(
        columns,
        vec![13, 13],
        "both tails start in the same column: {lines:#?}"
    );
}
...
fn padding_does_not_leak_across_a_section_boundary() {
    let mut rows = vec![
        ...
        Node::finding(Finding::new(FindingKind::ReadOnlyFilesystem {
    ...
    let lines = draw("devbox", &rows, StatusLine::Hints);
    let pressure: Vec<usize> = lines
        .iter()
        .filter(|l| l.contains("some avg60"))
        .map(|l| column_of(l, "some avg60"))
        .collect();
    let disk: Vec<usize> = lines
        .iter()
        .filter(|l| l.contains("remounted"))
        .map(|l| column_of(l, "remounted"))
        .collect();
    ...
    assert_eq!(
        disk,
        vec![11, 11],
        "the Disk section is padded to /nix, not to memory: {lines:#?}"
    );
}
...
fn app_tick_to_render_draws_a_failed_unit_on_a_real_frame() {
    ...
    app.tick(10_800_000, TIMESTAMP.to_string());
    ...
    for expected in [
        "Failed units (1)",
        "x restic-backup.service",
        "Degraded",
        "System",
    ] {
        assert!(
            lines.iter().any(|l| l.contains(expected)),
            "{expected:?} is missing from {lines:#?}"
        );
    }
}
...
fn the_frame_indents_rows_under_their_section_header() {
    ...
    assert!(
        header.starts_with(" masys - devbox"),
        "header is not inset: {header:?}"
    );

    let section = lines
        .iter()
        .find(|l| l.contains("Failed units"))
        .expect("the Failed units header");
    assert_eq!(
        section, " Failed units (1)",
        "section header sits at column 1"
    );

    let finding = lines
        .iter()
        .find(|l| l.contains("restic-backup.service"))
        .expect("the failed unit row");
    assert!(
        finding.starts_with("   x restic-backup.service"),
        "finding row is not indented: {finding:?}"
    );

    let overview = lines
        .iter()
        .find(|l| l.contains("degraded  ."))
        .expect("the overview row");
    assert!(
        overview.starts_with("   . degraded"),
        "overview row is not indented: {overview:?}"
    );

    // Every buffer is listed, current one included, so the row always
    // begins at the first.
    let hints = lines
        .iter()
        .find(|l| l.contains("[2] procs"))
        .expect("the footer hints");
    assert!(
        hints.starts_with(" [1] status"),
        "footer is not inset: {hints:?}"
    );
}
...
fn the_cursor_sits_at_the_middle_of_the_list_while_scrolling() {
    let rows: Vec<Node> = (0..60)
        .map(|n| Node::SectionHeader {
            title: format!("row-{n:02}"),
    ...
    let y = lines
        .iter()
        .position(|l| l.contains("row-30"))
        .expect("the selected row is on screen");

    let middle = list_top + list_height / 2;
    assert!(
        y.abs_diff(middle) <= 1,
        "selected row is at line {y}, middle of the list is {middle}\n{}",
        lines.join("\n")
    );
}
...
fn the_ends_of_the_list_clamp_rather_than_showing_blank() {
    ...
    assert_eq!(
        top.iter().position(|l| l.contains("row-00")),
        Some(list_top),
        "the first row stays at the top of the list"
    );

    let (bottom, _, _) = draw_selected(&rows, 59);
    assert!(
        bottom.iter().any(|l| l.contains("row-59")),
        "the last row is on screen"
    );
    assert!(
        !bottom.iter().any(|l| l.contains("row-00")),
        "and the list has actually scrolled"
    );
}
...
fn the_key_help_draws_over_the_buffer_and_stays_inside_the_frame() {
    ...
    assert!(
        lines.iter().any(|l| l.contains("cycle sort")),
        "the buffer's own keys are listed: {lines:#?}"
    );
    assert!(
        lines.iter().any(|l| l.contains("keys")),
        "the popup is titled: {lines:#?}"
    );
    for line in &lines {
        assert!(
            UnicodeWidthStr::width(line.as_str()) <= WIDTH as usize,
            "{line:?} runs past the frame"
        );
    }
}
...
/// The systemd buffer offers thirteen keys, 162 columns of them, and one
...
fn the_footer_wraps_rather_than_clipping_the_real_keymap() {
    ...
    for chord in [
        "r", "alt+r", "R", "S", "X", "e", "D", "F", "E", "M", "U", "l", "/",
    ] {
        assert!(
            footer.contains(&format!("[{chord}]")),
            "[{chord}] is not on screen: {footer}"
        );
    }
    ...
    assert!(
        lines.iter().any(|l| l.contains("System")),
        "the list survived: {footer}"
    );
}
...
fn a_footer_too_narrow_for_every_action_says_so() {
    let theme = Theme::default();
    let actions: Vec<KeyBinding> = (0..20)
        .map(|n| KeyBinding {
            chord: format!("{n}"),
            label: format!("action number {n}"),
            dimmed: false,
            active: false,
        })
        .collect();
    ...
    let first = lines
        .iter()
        .position(|l| l.contains("action number 0"))
    ...
    assert!(
        marked,
        "a clipped footer says so: {:?}",
        &lines[first..first + 3]
    );
    assert!(
        !lines[first + 3].contains("action number"),
        "the cap held: {:?}",
        &lines[first..]
    );
    for line in &lines {
...
fn the_filter_draws_under_the_header_not_over_the_rows() {
    ...
    assert!(
        lines[1].contains("/chrome"),
        "directly under the header: {:?}",
        lines[1]
    );
    assert!(
        lines[1].contains('\u{2588}'),
        "a cursor while typing: {:?}",
        lines[1]
    );
    assert!(
        lines.iter().any(|l| l.contains("System")),
        "the rows are still visible: {lines:#?}"
    );
}
...
fn a_single_match_is_counted_in_the_singular() {
    ...
    assert!(
        lines
            .iter()
            .any(|l| l.contains("1 match") && !l.contains("1 matches")),
        "{lines:#?}"
    );
}
...
fn no_matches_says_so_in_words() {
    ...
    assert!(
        !lines.iter().any(|l| l.contains("0 matches")),
        "a bare zero reads as a count, not an outcome: {lines:#?}"
    );
}
...
#[test]
fn a_paused_buffer_says_so_and_says_how_to_resume() {
    let rows = system_section();
    let lines = filtered("", None, true, &rows);
    let notice = lines
        .iter()
        .find(|l| l.contains("paused"))
        .expect("a pause notice");
    assert!(notice.contains("fold"), "it names the way out: {notice:?}");
    assert!(
        notice.contains('g'),
        "and the way to refresh anyway: {notice:?}"
    );
}

#[test]
fn a_running_buffer_carries_no_pause_notice() {
    let rows = system_section();
    let lines = filtered("", None, false, &rows);
...
fn opened_process(vars: usize) -> Vec<Node> {
    let detail = masys_domain::proc_detail::ProcDetail {
        ...
        env: (0..vars)
            .map(|n| (format!("VAR_{n}"), format!("value-{n}")))
            .collect(),
        ..Default::default()
    };
    vec![
        Node::Proc {
            proc: a_detail_proc(4242, "fish"),
            rate: None,
            cpu_seconds: 3,
            age_ms: Some(9_000),
            depth: 1,
...
fn an_open_process_always_renders_its_block_however_long_it_is() {
    for vars in [0usize, 5, 20, 21, 25, 60, 400] {
        ...
        assert!(
            lines.iter().any(|l| l.contains("cmdline")),
            "the block vanished at {vars} vars: {lines:#?}"
        );
        // And the row it belongs to is still on screen to own it.
        assert!(
            lines.iter().any(|l| l.contains("fish")),
            "at {vars} vars: {lines:#?}"
        );
    }
}
...
/// is out of scope here: this closes it for the Nix buffer alone.
#[test]
fn every_nix_row_type_reaches_the_frame() {
    let rows = vec![
        ...
        header(
            "System generations",
            SectionKind::Generations(ProfileKind::System),
            Some(1),
        ),
        Node::Generation {
    ...
    assert!(
        lines.iter().any(|l| l.contains("reboot pending")),
        "RebootPending never reached the frame: {lines:#?}"
    );
    assert!(
        lines
            .iter()
            .any(|l| l.contains("booted 41") && l.contains("current 42")),
        "{lines:#?}"
    );
    assert!(
        lines.iter().any(|l| l.contains("/nix 62%")),
        "NixStore never reached the frame: {lines:#?}"
    );
    assert!(
        lines.iter().any(|l| l.contains("5 system generations")),
        "{lines:#?}"
    );
    assert!(
        lines
            .iter()
            .any(|l| l.contains("gc") && l.contains("keep 14d")),
        "NixPolicy never reached the frame: {lines:#?}"
    );
    assert!(
        lines
            .iter()
            .any(|l| l.contains("42") && l.contains("26.11.20260804")),
        "Generation never reached the frame: {lines:#?}"
    );
    assert!(
        lines
            .iter()
            .any(|l| l.contains("nixpkgs") && l.contains("NixOS/nixpkgs")),
        "Input never reached the frame: {lines:#?}"
    );
}
...
fn a_transient_draws_its_title_groups_and_chords() {
    ...
    assert!(
        lines
            .iter()
            .any(|l| l.contains("Unit . restic-backup.service")),
        "the popup names its subject: {lines:#?}"
    );
    for heading in ["Runtime", "Persistence", "Inspect"] {
        assert!(
            lines.iter().any(|l| l.contains(heading)),
            "{heading} is missing: {lines:#?}"
        );
    }
    for label in [
        "start",
...
fn a_group_with_no_notes_pairs_its_rows_two_to_a_line() {
    ...
    let paired = lines
        ...
        .expect("a start row");
    assert!(
        paired.contains("stop"),
        "start and stop share a line: {paired:?}"
    );
    let second = lines
        .iter()
        .find(|l| l.contains("restart"))
        .expect("a restart row");
    assert!(
        second.contains("reload"),
        "restart and reload share a line: {second:?}"
    );
    assert!(
        lines
            .iter()
            .find(|l| l.contains("logs"))
            .is_some_and(|l| l.contains("properties")),
        "a two-row group is one line: {lines:#?}"
    );
}
...
fn the_ownership_guard_notes_the_group_and_keeps_every_row() {
    ...
    let heading = lines
        .iter()
        .find(|l| l.contains("Persistence"))
        .expect("a Persistence heading");
    assert!(
        heading.contains("! declared in nix"),
        "the group note trails the heading, marker and all: {heading:?}"
    );
    for label in ["enable", "disable", "mask"] {
        let row = lines
            .iter()
            .find(|l| l.contains(label))
            .unwrap_or_else(|| panic!("{label} is still listed: {lines:#?}"));
        assert!(
            row.contains("reverts on nixos-rebuild"),
            "{label} says why it is dimmed: {row:?}"
        );
    }
}
...
fn an_annotated_group_goes_one_row_to_a_line() {
    ...
    let enable = lines
        .iter()
        .find(|l| l.contains("enable"))
        .expect("an enable row");
    assert!(
        !enable.contains("disable"),
        "a noted row does not share its line: {enable:?}"
    );
}
...
fn a_dimmed_row_is_styled_dim_and_a_live_one_is_not() {
    ...
    assert!(
        !dim_at("restart"),
        "a row the guard does not dim is drawn normally"
    );
}
...
fn switches_are_drawn_above_the_actions_with_their_state_resolved() {
    ...
    let verbose = lines
        ...
        .expect("a verbose switch");
    let switch = lines
        .iter()
        .position(|l| l.contains("switch"))
        .expect("a switch action");
    assert!(
        verbose < switch,
        "switches are read before the actions they change: {lines:#?}"
    );

    assert!(
        lines[verbose].contains("[x]"),
        "an enabled switch is ticked: {:?}",
        lines[verbose]
    );
    assert!(
        lines
            .iter()
            .any(|l| l.contains("dry run") && l.contains("[ ]")),
        "a disabled switch is empty: {lines:#?}"
    );
    assert!(
        lines.iter().any(|l| l.contains("impure")),
        "an unsupported switch is still listed: {lines:#?}"
    );
}
...
fn a_transient_taller_than_the_frame_reports_what_it_could_not_show() {
    ...
    assert!(
        metrics.modal_height <= SHORT,
        "the popup is never taller than the frame: {metrics:?}"
    );
    assert!(
        metrics.modal_content > metrics.modal_height,
        "and it says how much it could not show: {metrics:?}"
    );

    let lines = buffer_lines(&terminal);
    assert!(
        !lines.iter().any(|l| l.contains("Group 11")),
        "content past the frame is clipped, not grown into: {lines:#?}"
    );
    for line in &lines {
        assert!(
            UnicodeWidthStr::width(line.as_str()) <= WIDTH as usize,
...
/// The `Input` sub-step draws its prompt, what has been typed, and a
/// cursor block - and nothing else, because text entry is all it is.
///
/// It carried a candidate picker until 2026-08-28, tested three ways
/// against a `CandidateList` no caller ever built. The tests passed and
/// the popup was still a prompt: `App::modal` had one construction site
/// and it passed `candidates: None`. Covering a shape is not the same as
/// covering a path to it.
#[test]
fn an_input_draws_its_prompt_and_typed_text_and_nothing_else() {
    ...
    assert!(
        lines.iter().any(|l| l.contains("retention: 30\u{2588}")),
        "the prompt, the typed text and a cursor: {lines:#?}"
    );
    assert!(
        lines.iter().any(|l| l.contains("input")),
        "the popup is titled for what it is: {lines:#?}"
    );
}

/// Every prompt masys opens is free text, so none of them grows rows
/// under the typed line. The three that exist - a search query, an option
/// name and a generation spec - are checked here together, because the
/// bug this replaces was a popup that drew rows nothing could produce.
#[test]
fn no_prompt_invents_rows_under_what_was_typed() {
    for (prompt, typed) in [
        ("query", "ripgrep"),
        ("option", "services.openssh.enable"),
        ("delete generations", "+5"),
    ] {
        let lines = draw_modal_frame(ModalView::Input { prompt, typed }, WIDTH, HEIGHT);

        assert!(
            lines
                .iter()
                .any(|l| l.contains(&format!("{prompt}: {typed}"))),
            "{prompt} draws what was typed: {lines:#?}"
        );
        let drawn = lines
            .iter()
            .filter(|l| l.contains(typed) || l.contains("no matches"))
            .count();
        assert_eq!(
            drawn, 1,
            "{prompt} draws one line and invents nothing under it: {lines:#?}"
        );
    }
}
...
fn a_group_note_is_drawn_as_written_without_an_invented_marker() {
    ...
    assert!(
        !heading.contains('!'),
        "the renderer adds no marker of its own: {heading:?}"
    );
}
...
fn app_key_to_render_draws_the_unit_popup_on_a_real_frame() {
    ...
    assert!(
        lines.iter().any(|l| l.contains("Unit .")),
        "the popup names its unit: {lines:#?}"
    );
    for expected in [
        "Runtime",
        "start",
...
fn a_popup_is_wide_enough_for_its_title() {
    ...
    let title = lines
        .iter()
        .find(|l| l.contains("a-very-long-unit-name.service"))
        .expect("a titled popup");
    assert!(
        title.contains("a-very-long-unit-name.service "),
        "the title keeps the padding it was written with: {title:?}"
    );
    assert!(
        title.ends_with('\u{2510}'),
        "and the border closes after it: {title:?}"
    );
}
