...
pub fn parse_stat(text: &str) -> Result<RawStat, MasysError> {
    let open = text
        .find('(')
        .ok_or_else(|| MasysError::System("stat: no comm".into()))?;
    let close = text
        .rfind(')')
        .ok_or_else(|| MasysError::System("stat: unterminated comm".into()))?;
    if close < open {
    ...
    let field = |n: usize| -> Result<&str, MasysError> {
        rest.get(n - 3)
            .copied()
            .ok_or_else(|| MasysError::System(format!("stat: missing field {n}")))
    };
    let num = |n: usize| -> Result<u64, MasysError> {
        field(n)?
            .parse()
            .map_err(|_| MasysError::System(format!("stat: field {n} is not a number")))
    };
    ...
    Ok(RawStat {
        ...
        nice: field(19)?
            .parse()
            .map_err(|_| MasysError::System("stat: nice is not a number".into()))?,
