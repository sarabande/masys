...
fn the_unified_hierarchy_is_a_single_line() {
    ...
    assert_eq!(
        parse_cgroup(text).as_deref(),
        Some("/user.slice/user-1000.slice/user@1000.service/app.slice/kitty-3002-8.scope")
    );
}
...
fn an_unrecognised_layout_falls_back_to_the_first_controller() {
    assert_eq!(
        parse_cgroup("7:blkio:/some/path\n").as_deref(),
        Some("/some/path")
    );
}
...
fn a_path_containing_colons_survives() {
    ...
    assert!(
        parse_cgroup(text)
            .expect("a path")
            .ends_with("pci\\x2d0000:00:17.0.device")
    );
}
